...
pub struct MintQuoteCustomResponse<Q> {
    // NUT #04: `quote` **SHOULD** be UUID v7 with all 74 variable bits generated by a CSPRNG and **MUST** remain a secret between user and mint and **MUST NOT** be derivable from the payment request.
    ...
    // NUT #04: Mints **MUST** include `amount_paid`, `amount_issued`, and `updated_at` in all mint quote responses. `amount_paid` and `amount_issued` **MUST** be non-negative integers, and `amount_issued` **MUST NOT** exceed `amount_paid`.

    /// Amount that has been paid
    pub amount_paid: Amount,
    // NUT #04: Mints **MUST** update `updated_at` whenever `amount_paid` or `amount_issued` changes. Mints
    // **MUST** ensure that `updated_at` monotonically increases for each quote, even if multiple updates
    // occur within the same timestamp resolution. Wallets that receive multiple responses for the same
    // quote **MUST NOT** replace locally stored quote data with a response whose `updated_at` is lower
    // than the latest processed value for that quote. Wallets **MUST NOT** decrease locally stored
...
mod tests {
    ...
    fn test_onchain_settings_nested_options_round_trip() {
        // NUT26 spec format: confirmations nested inside "options"
        let json_str = r#"{
            "method": "onchain",
            "unit": "sat",
            "min_amount": 1000,
